...
export default class HelloWorldAsClass extends React.Component {
  ...
  render () {
    return (
      <div>
        <input onChange={e => this.context.store.dispatch(Actions.updateGreeting(e.target.value))} value={this.state.greeting}/>
        <p>Hello, {this.state.greeting}</p>
        <button onClick={() => this.context.store.dispatch(Actions.resetGreeting())}>Clear</button>
      </div>
    )
  }
}