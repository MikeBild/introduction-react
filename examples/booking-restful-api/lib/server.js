...
app.get('/search', (req, res, next) => {
  ...
  Promise.all([
    databases.flights.allDocs({include_docs: true})
      .then(result => result.rows.map(x => Object.assign({}, x.doc, {source: 'Flight'}))),
    databases.hotels.allDocs({include_docs: true})
      .then(result => result.rows.map(x => Object.assign({}, x.doc, {source: 'Hotel'}))),
    databases.cars.allDocs({include_docs: true})
      .then(result => result.rows.map(x => Object.assign({}, x.doc, {source: 'Car'}))),
  ])
  .then(results => results.reduce((s, n) => s.concat(n), []))
  .then(results => results.filter(x => location ? x.location === location : true))
