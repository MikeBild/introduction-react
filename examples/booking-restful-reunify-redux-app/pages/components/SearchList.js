        ...
        <SearchInputContainer>
          <div>
            <label>
              ...
              Search by location (MUC, BER)
            </label>
            <br />
            <SearchInput
              type="search"
              onBlur={e => fetchSearch({ location: e.target.value })}
            />
          </div>
        </SearchInputContainer>
        <hr />
        {search && search.map((x, i) => <SearchItem key={i} {...x} />)}
        {error &&
          error.message && <FailureMessage>{error.message}</FailureMessage>}
      </div>
    );
  }
}
